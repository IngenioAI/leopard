...
def define_G_Adain(input_nc, output_nc, latent_size, ngf, netG, n_downsample_global=2, n_blocks_global=4,
...
def define_G_Adain_Mask(input_nc, output_nc, latent_size, ngf, netG, n_downsample_global=2, n_blocks_global=4,
                        norm='instance', gpu_ids=[]):
    norm_layer = get_norm_layer(norm_type=norm)
...
def define_G_Adain_Upsample(input_nc, output_nc, latent_size, ngf, netG, n_downsample_global=2, n_blocks_global=4,
                            norm='instance', gpu_ids=[]):
    norm_layer = get_norm_layer(norm_type=norm)
    netG = Generator_Adain_Upsample(input_nc, output_nc, latent_size, ngf, n_downsample_global, n_blocks_global,
                                    norm_layer)
    print(netG)
    if len(gpu_ids) > 0:
        assert (torch.cuda.is_available())
...
def define_G_Adain_2(input_nc, output_nc, latent_size, ngf, netG, n_downsample_global=2, n_blocks_global=4,
                     norm='instance', gpu_ids=[]):
    norm_layer = get_norm_layer(norm_type=norm)
...
class Generator_Adain(nn.Module):
    def __init__(self, input_nc, output_nc, latent_size, ngf=64, n_downsampling=2, n_blocks=4,
        ...
        mult = 2 ** n_downsampling
        for i in range(n_blocks):
            BN += [ResnetBlock_Adain(ngf * mult, latent_size=latent_size, padding_type=padding_type,
...
class Generator_Adain_2(nn.Module):
    def __init__(self, input_nc, output_nc, latent_size, ngf=64, n_downsampling=2, n_blocks=4,
        ...
        for i in range(n_downsampling):
            mult = 2 ** (n_downsampling - i)
            Dec += [UpBlock_Adain(dim_in=ngf * mult, dim_out=int(ngf * mult / 2), latent_size=latent_size,
