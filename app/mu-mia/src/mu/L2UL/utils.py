...
class NormalizeLayer(torch.nn.Module):
    ...
    def forward(self, input: torch.tensor):
        ...
        means = self.means.to(device).repeat((batch_size, height, width, 1)).permute(0, 3, 1, 2)
        sds = self.sds.to(device).repeat((batch_size, height, width, 1)).permute(0, 3, 1, 2)
...
def adv_attack(args, model, device, train_loader, adversary, unlearn_k, num_adv_images = None):
    ...
    for i in range (num_iters):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            attack_label = torch.rand(data.shape[0]).to(device) * 10
            attack_label = attack_label.to(torch.long)
            attack_label = torch.where(attack_label == target, (torch.rand(data.shape[0]).long().to(device)*10 + 10) // 2, attack_label)
